        ...
        case (ControlCommand::CommandType::GetVariable): {
            switch (static_cast<ControlCommand::Variable>(buffer.buffer[1])) { // Switch variable
                case (ControlCommand::Variable::PowerMode): {
                    auto response = ReturnVariableResponse(ControlResponse::Variable::PowerMode, 
...
Vector3<float, uint8_t> calculateAngles(const Vector3<float, uint8_t>& eulerAngles) {
    ...
    return {
        {-2 * atan2f((sqrt2 * (sqrt2 * sinB - util::sign(a) * sqrtf(2 - 2 * cos2A * cos2B - 2 * sin2B))), (2 * (sqrtf(1 - 2 * sin2B) + cosA * cosB)))},
        {asinf(sinM2)},
        {F_PI_4 + signG * 2 * atan2f((std::sqrt(2 - 2 * cos2B * sin2G - 2 * sin2B) + signG * 1), (sqrtf(1 - 2 * sin2B) - sqrt2 * cosB * sinG))}
    };
}
