...
void data::edit(const uint8_t& addr, uint8_t& buf, uint8_t len) {
    if (written) {
        util::copy(reinterpret_cast<uint32_t*>(rowCopy), reinterpret_cast<const uint32_t*>(data::NVM_DATA), sizeof data::NVM_DATA / 4);
        written = false;
    }

    util::copy(rowCopy + (&addr - data::NVM_DATA), &buf, len);
}
...
void data::write() {
    ...
    for (uint8_t row{0}; row < sizeof (data::NVM_DATA) / FLASH_PAGE_SIZE / 4; ++row) {
        ...
        for (uint8_t i{0}; i < 4; ++i) {
            ...
            util::copy((uint32_t*)(data::NVM_DATA + offset), (uint32_t*)(rowCopy + offset), FLASH_PAGE_SIZE / 4);
