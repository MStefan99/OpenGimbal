...
void applyTorque(uint16_t angle, uint8_t power, bool counterclockwise = true) {
    ...
    bldc::applyTorque(counterclockwise == nvm::options->counterclockwise? eAngleSigned + quarterRevolution: eAngleSigned + (fullRevolution - quarterRevolution), power);
}
...
bool checkCalibration(uint16_t phaseOffset, uint8_t polePairs, bool counterclockwise) {
    ...
    for (uint16_t i {0}; i < 4096; ++i) {
        uint16_t target = util::mod(static_cast<uint16_t>((counterclockwise? - step * i : step * i) + startAngle), fullRevolution);
        ...
        bldc::applyTorque((difference > 0) == counterclockwise? eAngleSigned + quarterRevolution : eAngleSigned + (fullRevolution - quarterRevolution), 
