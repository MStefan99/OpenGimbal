...
void pwm::init() {
	...
	PORT_REGS->GROUP[0].PORT_WRCONFIG = PORT_WRCONFIG_PINMASK(0x1 << 14u | 0x1 << 15u) | PORT_WRCONFIG_PMUXEN(1)
	                                  | PORT_WRCONFIG_PMUX(MUX_PA14E_TC1_WO0) | PORT_WRCONFIG_WRPMUX(1)
	                                  | PORT_WRCONFIG_WRPINCFG(1);
	PORT_REGS->GROUP[0].PORT_WRCONFIG = PORT_WRCONFIG_PINMASK(0x1 << 14u | 0x1 << 15u) | PORT_WRCONFIG_PMUXEN(1)
	                                  | PORT_WRCONFIG_PMUX(MUX_PA30E_TC2_WO0) | PORT_WRCONFIG_WRPMUX(1)
	                                  | PORT_WRCONFIG_WRPINCFG(1) | PORT_WRCONFIG_HWSEL(1);

	TC2_REGS->COUNT8.TC_CTRLA = TC1_REGS->COUNT8.TC_CTRLA =
	    TC_CTRLA_ENABLE(1) | TC_CTRLA_MODE_COUNT8 | TC_CTRLA_WAVEGEN_NPWM | TC_CTRLA_PRESCALER_DIV1;
}
